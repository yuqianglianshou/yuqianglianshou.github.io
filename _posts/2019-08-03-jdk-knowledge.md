...
| JDK版本            | 发布时间       | 生命周期（LTS与否） | 主要特性                                          |
| ---------------- | ---------- | ----------- | --------------------------------------------- |
| **JDK 1.0\~1.4** | 1996\~2002 | 非LTS        | Java初始版本，加入集合框架、断言、异常链等。                      |
| **JDK 5**（1.5）   | 2004       | 非LTS        | 泛型、注解、增强for循环、自动装箱、枚举。                        |
| **JDK 6**（1.6）   | 2006       | 非LTS        | 性能提升、脚本语言支持、JDBC 4.0。                         |
| **JDK 7**        | 2011       | 非LTS        | Switch支持String、菱形操作符（<>）、NIO.2。               |
| **JDK 8** ⭐      | 2014       | ✅LTS        | Lambda表达式、Stream API、默认方法、Optional、Nashorn引擎。 |
| **JDK 9**        | 2017       | 非LTS        | 模块化系统（JPMS）、JShell。                           |
| **JDK 10**       | 2018       | 非LTS        | 局部变量类型推断（`var`）。                              |
| **JDK 11** ⭐     | 2018       | ✅LTS        | 移除Java EE模块、引入HttpClient API、运行单个Java文件。      |
| **JDK 12\~16**   | 2019\~2021 | 非LTS        | Switch表达式（预览）、记录类（record）、密封类等逐步添加。           |
| **JDK 17** ⭐     | 2021       | ✅LTS        | 永久代彻底移除、正式支持密封类、记录类、模式匹配等。                    |
| **JDK 18**       | 2022       | 非LTS        | 简化API更新、预览的代码段API（Snippet API）。               |
| **JDK 19**       | 2022       | 非LTS        | 虚拟线程（预览），结构化并发（预览）。                           |
| **JDK 20**       | 2023       | 非LTS        | 继续增强虚拟线程等预览功能。                                |
| **JDK 21** ⭐     | 2023       | ✅LTS        | 正式引入虚拟线程、字符串模板（预览）、模式匹配。                      |
| **JDK 22+**      | 2024\~     | 非LTS        | 持续引入项目 Panama、Loom、Amber 等特性。                 |
...
标准类库（java.util, java.io 等）

工具链（javadoc、jar、jlink 等）

## **JDK 在 Android 项目中的作用：**

| 功能             | 作用                                   |
| -------------- | ------------------------------------ |
| 编译 Java 源码     | 用 `javac` 编译 `.java` 文件为 `.class` 文件 |
| 运行 Gradle 构建工具 | Gradle 和 AGP 自身就是基于 Java 实现的，需要 JDK  |
| 运行其他构建工具       | 如 lint、Kotlin 编译器等都需要 JDK 支持         |
| 使用标准类库         | 项目中的 Java API 来自 JDK（部分）             |
